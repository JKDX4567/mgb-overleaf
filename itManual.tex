...
% Make some common Unicode glyphs work under pdfLaTeX in listings:
\lstset{
  literate=
    {–}{{-}}1   % en-dash → -
    {—}{{--}}1  % em-dash → --
    {…}{{...}}1
    {─}{{-}}1   % U+2500 box drawing light horizontal
    {│}{{|}}1   % U+2502 box drawing light vertical
    {«}{{<<}}1  {»}{{>>}}1
}
...
\hypersetup{
  linktoc=all,        % make ToC/LoF/LoT entries clickable
  pdfborder={0 0 0}   % no ugly boxes (also avoids some viewer quirks)
}
\usepackage{bookmark}
...
\cleardoublepage
\pagenumbering{roman}
\setcounter{tocdepth}{2}   % sections in ToC; bump to 3 for subsections

% Make bookmark + link targets (helps some viewers)
\phantomsection
...
% If you ever add \lstlistoflistings:
% \cleardoublepage
% \phantomsection
% \pdfbookmark[0]{List of Listings}{lol}
% \lstlistoflistings
...
\setlength{\headheight}{26.1pt}
\addtolength{\topmargin}{-11.6pt} % as suggested by the warning
